...
public class TileBuilding extends TypedTile<BuildingType> {
	...
	public enum BuildingType implements TileTypeSized {
		NORMAL_HOUSE("house", SpriteTheme.NONE, 8, 4),
		FAIRY_HOUSE("house_fairy", SpriteTheme.SCHALLENBERGE, 3, 3),
		DWARF_BARRACKS("barracks", "barrack_dwarf", SpriteTheme.DWARF_CITY, 9, 5),
		DWARF_HOUSE("house_dwarf", SpriteTheme.DWARF_CITY, 8, 5),
		DWARF_HOUSE_2("house_dwarf_2", SpriteTheme.DWARF_CITY, 7, 8);
		
		private final String sprite, name;
		private final SpriteTheme theme;
		private final double width, height;
		
		private BuildingType(final String sprite, final String name, final SpriteTheme theme, final double width, final double height) {
			this.sprite = sprite;
			this.name = name;
			this.theme = theme;
			this.width = width;
		...
		private BuildingType(final String name, final SpriteTheme theme, final double width, final double height) {
			this("house", name, theme, width, height);
		}
